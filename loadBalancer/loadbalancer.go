...
func (server *Server) isAlive() bool {
	...
	if err != nil {
		...
		log.Println("Health check for error:", err)
		server.Alive = false
		return false
	}
	log.Println(server.Route, "isAlive")
	server.Alive = true
	return true
}
...
func (serverList *ServerList) init(serverRoutes []string) {
	log.Println("Creating the server list for routes :", serverRoutes)
	...
	for _, serverRoute := range serverRoutes {
		...
		director := func(req *http.Request) {
			req.Header.Add("X-Forworded-Host", req.Host)
			req.Header.Add("X-Origin-Host", origin.Host)
			req.URL.Scheme = "http"
			req.URL.Host = origin.Host
		}
		localServer.ReverseProxy = &httputil.ReverseProxy{Director: director}
		log.Println("Server", localServer, "Added Server List")
		serverList.Servers = append(serverList.Servers, localServer)
	}
	serverList.Latest = -1
	log.Println("Successfully created the server list:", serverList)
